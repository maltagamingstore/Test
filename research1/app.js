(() => {
  ...
  function comparisons(){
    if(!data)return;
    const rows=[...data.results].sort((a,b)=>(a.name==='immediate_exit_v1'?-1:b.name==='immediate_exit_v1'?1:0));
    if(!rows.length && data.preliminary){
      const p=data.preliminary, label=id=>names[id]||(id+' · '+((data.mutations||[]).find(m=>m.id===id)?.name||id));
      $('comparison').innerHTML='<div class="sample-banner"><strong>Preliminary · Recorded market data</strong><p>'+escape(new Date(p.window_start).toLocaleString())+' → '+escape(new Date(p.window_end).toLocaleString())+'</p><p>'+p.episode_markets+' historical initial-fill market episodes from a captured '+p.membership_markets+'-market cohort. This does not cover every market.</p><ul>'+p.limitations.map(x=>'<li>'+escape(x)+'</li>').join('')+'</ul></div><div class="table-scroll"><table><thead><tr><th>Rule</th><th>Completed / episodes</th><th>Paired for $ comparison</th><th>Unresolved / shares</th><th>Loss estimate*</th><th>Immediate exit on same episodes*</th></tr></thead><tbody>'+p.rows.map(r=>'<tr><td>'+escape(label(r.name))+(r.name==='R020'?'<br><small>Partial: reward-budget trigger unavailable</small>':'')+'</td><td>'+r.completed+' / '+r.episodes+'</td><td>'+r.paired_completed+'</td><td>'+r.censored+' / '+Number(r.remaining_shares).toFixed(2)+'</td><td>'+money(r.trading_loss)+'</td><td>'+money(r.paired_control_loss)+'</td></tr>').join('')+'</tbody></table></div><p class="footnote">* Dollars on each rule’s '+ 'completed, fee-estimated paired subset only. Unresolved positions are excluded from these dollar totals and remain visible above. Rows can contain different episodes: do not rank their totals against one another. Missed rewards and total cost are unknown. This is not whole-bot profit or a winner verdict.</p>';
      return;
    }
  ...
  function render(doc,fallback=false){
    ...
    $('mutation-list').innerHTML=mutations.map(r=>'<article class="mutation"><span class="step">'+escape(r.id)+' / '+escape(r.family.replaceAll('_',' '))+'</span><h3>'+escape(r.name)+'</h3><p>'+escape(r.description)+'</p><span class="mutation-state">'+(doc.preliminary?(r.id==='R020'?'Recorded check · reward-budget trigger pending':'Recorded snapshot check available'):'Data available · replay integration unfinished')+'</span></article>').join('');
